...
std::optional<Shape> MakeCircle(const std::vector<double> &v) {
    ...
    return RequireSize(v, 3)
           .and_then([](const auto &v){return v[2] > 0. ? std::make_optional(v) : std::nullopt; })
...
std::optional<Shape> MakeRectangle(const std::vector<double> &v) {
    // clang-format off
    return RequireSize(v, 4)
           .and_then([](const auto& v){ return (v[2] > 0 && v[3] > 0) ? std::make_optional(v) : std::nullopt; })
           .transform([](const auto &v) {
               return Rectangle{{v[0], v[1]}, v[2], v[3]};
           });
    // clang-format on
...
std::optional<Shape> MakePolygon(const std::vector<double> &v) {
    ...
    return RequireSize(v, 4)
           .and_then([](const auto &v) { return v[2] > 0 ? std::make_optional(v) : std::nullopt; })
           .and_then([](const auto &v) { return ParseDouble(std::to_string(v[3])); })
           .and_then([&v](double s) { int sides = static_cast<int>(v[3]);
                                      if (sides != s || sides < 3) { // должно быть целым и >=3
